...
namespace DeskApp
{
	sealed class rms_var
	{
		...
		public static string TimeBlock;
		public static int FoodInputMode = 3; //0 菜品编码 1 菜品拼音码 2 菜品类别编码 3 = 菜品类别名称
		
		//窗口
		public static mainform mainfrm;
		public static frmAppoint frmAppnt;
		public static InputBill frmInputBill;
		public static frmTableStatus frmTableST;
		public static frmBrsSpecialFood frmbrssfod;
		public static frmBrsCurrentFood frmbrscfod;
		
		public static frmCustomer frmCust;
		
		public static frmQuerySignPay frmSP;
		public static frmCustomerFreq frmCF;
		...
		public static ManualResetEvent e=new ManualResetEvent(true);
...
//				reg.Close();
		...
		public static bool ConnectSvr()
		{
            try
            {
                //客户端不需要定义通道
                //按指定的URI创建远程对象代理
                string RemoteRMSServerName = ReadStrfromReg("RemoteRMSServerName", "localhost");
                string RemoteRMSServerPort = ReadStrfromReg("RemoteRMSServerPort", "8023");
                svrobj = (CRmsSvr.CRMSServer)(Activator.GetObject(typeof(CRmsSvr.CRMSServer), "tcp://" + RemoteRMSServerName + ":" + RemoteRMSServerPort + "/RMSServer"));
            ...
            {
                MessageBox.Show("无法创建代理:"+e.Message);
            }
            return false;
		}
		...
		public static string GetTableTypeCode(string Typename)
		{
			...
			try
			{
				...
				DataSet dbDS1 = new DataSet();
				dbDA1.Fill(dbDS1, "t1");
				ret=dbDS1.Tables[0].Rows[0][0].ToString();
		...
		//返回雇员默认折扣
		public static double GetEmplDiscount(string empid)
		{
			System.Data.SqlClient.SqlConnection conn1 = new System.Data.SqlClient.SqlConnection();
			conn1.ConnectionString = ConnStr;
			try
			{
				SqlCommand selectCMD1 = new SqlCommand("SELECT Discount FROM Employee where empid='" + empid + "'", conn1);
				selectCMD1.CommandTimeout = 30;
				SqlDataAdapter dbDA1 = new SqlDataAdapter();
				dbDA1.SelectCommand = selectCMD1;
				conn1.Open();
				DataSet dbDS1 = new DataSet();
				dbDA1.Fill(dbDS1, "t1");
				return double.Parse(dbDS1.Tables[0].Rows[0][0].ToString());
		...
		public static bool TableConsumeBillExists(string tableno)
		{
			...
			try
			{
				...
				DataSet dbDS1 = new DataSet();
				dbDA1.Fill(dbDS1, "t1");
				if (Convert.ToInt32(dbDS1.Tables[0].Rows[0][0].ToString()) > 0) //具备消费单据
				{
					return true;
				}
				else
		...
		public static int GetOpenTablePsn(string tableno)
		{
			...
			try
			{
				...
				DataSet dbDS1 = new DataSet();
				dbDA1.Fill(dbDS1, "t");
				ret=Convert.ToInt32(dbDS1.Tables[0].Rows[0][0].ToString());
		...
		public static bool UpdateAppointTable(string timeblockcode)
		{
			...
			try
			{
				...
				for (int i = 0; i <= dbDS1.Tables[0].Rows.Count - 1; i++)
				{
					...
					{
						MessageBox.Show("在当前 " + currentdate + " " + TimeBlock + " 预订到期的 " + dbDS1.Tables[0].Rows[i][0] + " 号桌台正在使用，更新预订操作不能完成！");
		...
		//返回雇员名称
		public static string GetEmpName(string operatorid)
		{
			System.Data.SqlClient.SqlConnection conn = new System.Data.SqlClient.SqlConnection();
			conn.ConnectionString = ConnStr;
			try
		...
		public static double GetFoodRemainQuantity(string foodcode)
		{
			...
			try
			{
				...
				DataSet dbDS1 = new DataSet();
				dbDA1.Fill(dbDS1, "t");
				ret=Convert.ToDouble(dbDS1.Tables[0].Rows[0]["remain"].ToString());
		...
		public static string GetTableMaxPeopleNumber(string tableno)
		{
			...
			try
			{
				...
				DataSet dbDS1 = new DataSet();
				dbDA1.Fill(dbDS1, "t");
				ret=dbDS1.Tables[0].Rows[0]["peoplenumber"].ToString ();
		...
		public static string GetTableOpenTime(string tableno)
		{
			...
			try
			{
				...
				DataSet dbDS1 = new DataSet();
				dbDA1.Fill(dbDS1, "t");
				ret=dbDS1.Tables[0].Rows[0]["begintime"].ToString();
		...
		public static bool CurrentTableExistsFood(string tableno, string foodcode)
		{
			...
			try
			{
				...
				DataSet dbDS1 = new DataSet();
				dbDA1.Fill(dbDS1, "t");
				if (Convert.ToInt32(dbDS1.Tables[0].Rows[0][0].ToString ())> 0) //已经存在
				{
					return true;
				}
				else
		...
		//更新桌台人数
		public static void UpdateTablePsn(string tableno, string psn)
		{
			System.Data.SqlClient.SqlConnection conn = new System.Data.SqlClient.SqlConnection();
			conn.ConnectionString = ConnStr;
			try
			{
				...
				SQLCMD.Connection = conn;
				SQLCMD.CommandTimeout = 30;
				SQLCMD.CommandText = "update tablestatus set peoplenumber=" + psn + " where tableno='" + tableno + "'";
				conn.Open();
				SQLCMD.ExecuteNonQuery();
			}
			catch (Exception ex)
		...
		public static string GetTableLowPay(string tableno)
		{
			...
			try
			{
				...
				DataSet dbDS1 = new DataSet();
				dbDA1.Fill(dbDS1, "t");
                return string.Format("{0:N2}", System.Convert.ToDouble(dbDS1.Tables[0].Rows[0][0]));
			}
			catch (Exception ex)
			{
				MessageBox.Show(ex.ToString());
		...
		public static string GetTableServerPay(string tableno, double ConsumePrice)
		{
			...
			try
			{
				...
				{
					if (dbDS1.Tables[0].Rows[0]["serverstate"].ToString() == "0") //收取固定服务费
					{
                        return string.Format("{0:N2}", System.Convert.ToDouble(dbDS1.Tables[0].Rows[0]["server_pay"]));
					}
					else if (dbDS1.Tables[0].Rows[0]["serverstate"].ToString() =="1") //按消费额度收取服务费
					{
		...
		public static string GetTableOverTimePay(string tableno, double ConsumePrice)
		{
			...
			try
			{
				...
				if (needpayOvertime) //需要支付超时费，检查支付类型
				{
					...
					difftime =Convert.ToInt32(DateDiff(DateInterval.Hour, opentime, DateTime.Now)); //返回小时差
                    ...
                    overtimetype = dbDS1.Tables[0].Rows[0]["overtimetype"].ToString(); //返回超过时限是分钟还是小时 0=分钟 1=小时
					overtimenum = System.Convert.ToDouble(dbDS1.Tables[0].Rows[0]["overtimenum"]); //超过时限
                    overtimestate = dbDS1.Tables[0].Rows[0]["overtimestate"].ToString(); //支付类型 0=所超时限收取超时费 1=消费百分比收取
                    pertype = dbDS1.Tables[0].Rows[0]["pertype"].ToString();
						...
						if (difftime * 60 > overtimenum) //超时了 overtimenum 的单位是分钟
						{
							...
							if (overtimestate == "0") //收取固定费用
							{
								if (pertype == "0") //加收方法 - 分钟
								{
									...
									ret=string.Format("{0:N2}",overprice);
								}
		...
		public static string GetCurrentTableSerialNo(string tableno)
		{
			...
			try
			{
				...
				if (dbDS1.Tables[0].Rows.Count > 0)
				{
					ret=dbDS1.Tables[0].Rows[0]["serialno"].ToString();
				}
				else
				{
		...
		public static string GetFoodPrice(string foodcode, bool suit)
		{
			...
			try
			{
				...
				if (dbDS1.Tables[0].Rows.Count > 0)
				{
					...
					{
						ret=string.Format("{0:N2}",System.Convert.ToDouble(dbDS1.Tables[0].Rows[0]["specialprice"].ToString()));
					}
					else
                    if (dbDS1.Tables[0].Rows[0]["currentfood"]== "1") //是时价菜
					{
                        ret=string.Format("{0:N2}",System.Convert.ToDouble(dbDS1.Tables[0].Rows[0]["currentprice"].ToString()));
					}
					else
					{
                        ret=string.Format("{0:N2}",System.Convert.ToDouble(dbDS1.Tables[0].Rows[0]["price"].ToString()));
					}
				}
				else
				{
		...
		public static bool DeleteBill(string tableno, string billno, string delrsn)
		{
			...
			try
			{
				...
				for (int i = 0; i <= dbDS.Tables[0].Rows.Count - 1; i++)
				{
					...
					{
						DeleteFood(tableno, billno,dbDS.Tables[0].Rows[i][0].ToString(),(System.Convert.ToDouble(dbDS.Tables[0].Rows[i][1]) - System.Convert.ToDouble(dbDS.Tables[0].Rows[i][2])).ToString(), "删除单据操作所包含的套餐，单据号：" + billno,dbDS.Tables[0].Rows[i][3].ToString(), false);
					}
				}
				//删除套餐
		...
		//当前菜品是否允许打折
		public static bool IsAllowDiscountFood(string foodcode)
		{
			System.Data.SqlClient.SqlConnection conn1 = new System.Data.SqlClient.SqlConnection();
			conn1.ConnectionString = ConnStr;
			try
			{
				SqlCommand selectCMD1 = new SqlCommand();
				selectCMD1.Connection = conn1;
				selectCMD1.CommandText = "SELECT count(*) FROM foodlist where foodcode='" + foodcode + "' and (nodiscount='1' or specialfood='1')";
				selectCMD1.CommandTimeout = 30;
				SqlDataAdapter dbDA1 = new SqlDataAdapter();
				dbDA1.SelectCommand = selectCMD1;
				conn1.Open();
		...
		public static string GetFoodsDiscountCost(string tableno, double foodcost, double discountrate)
		{
			...
			try
			{
				...
				for (int i = 0; i <= dbDS1.Tables["t"].Rows.Count - 1; i++)
				{
                    if (IsConsumeBill(dbDS1.Tables["t"].Rows[i]["billtypecode"].ToString(), "")) //确定统计的是消费单
					{
						...
						//discountrate = CDbl(dbDS1.Tables("t1").Rows(0).Item("discountrate"))
						//检测当前菜品是否允许打折（除不优惠和特价菜以外的菜品为允许打折）
						if (IsAllowDiscountFood(foodcode))
						{
							DiscountPrice = DiscountPrice + foodprice * (1 - discountrate);
						}
					}
				}
		...
		//检测是否抹零、四舍五入
		//0：数值不变；1：自动抹零；2：四舍五入
		public static string FixCost()
		{
			System.Data.SqlClient.SqlConnection conn1 = new System.Data.SqlClient.SqlConnection();
			conn1.ConnectionString = ConnStr;
			try
