...
{
	public class frmStorageInventory : System.Windows.Forms.Form
	{
		...
		private DataTable LoadMaterialsInventory()
		{
			...
			try
			{
				...
				selectCMD.CommandText = "SELECT MaterialCode,MaterialName,Unit,Quantity,totalprice/quantity as Price,totalprice FROM view_Storage_Inventory " + "where deptname='" + CurrentStorageDeptName + "' and yearmonth='" + rms_var.GetSysSetup("Begindate") + "'";
		...
		private bool SaveMaterials(string yearmonth)
		{
			...
			try
			{
				...
				selectCMD.CommandTimeout = 30;
				conn.Open();
				selectCMD.CommandText = "delete from storage_inventory where yearmonth='" + yearmonth + "'";
				selectCMD.ExecuteNonQuery();
				for (int i = 0; i <= dgMaterials.VisibleRowCount - 1; i++)
				{
					selectCMD.CommandText = "insert into storage_inventory (yearmonth,MaterialCode,Quantity,Price,totalprice) " + "values (" + "'" + yearmonth + "'" + "," + "'" + dgMaterials[i, 0] + "'" + "," + dgMaterials[i, 3].ToString() + "," + dgMaterials[i, 4].ToString() + "," + dgMaterials[i, 5].ToString() + ")";
		...
		private bool DeleteMaterial(string materialcode)
		{
			...
			try
			{
				...
				selectCMD.CommandTimeout = 30;
				conn.Open();
				selectCMD.CommandText = "delete from storage_inventory where materialcode='" + materialcode + "' and yearmonth='" + rms_var.GetSysSetup("Begindate") + "'";
				selectCMD.ExecuteNonQuery();
				return true;
			}
			catch (Exception ex)
		...
		private void frmStorageInventory_Closed(object sender, System.EventArgs e)
		{
			rms_var.frmStorageInvent = null;
		}
		...
		private void ToolBar1_ButtonClick(System.Object sender, System.Windows.Forms.ToolBarButtonClickEventArgs e)
		{
			switch (ToolBar1.Buttons.IndexOf(e.Button))
			{
				...
				case 2:
					if (dgMaterials.VisibleRowCount > 0)
					{
						if (SaveMaterials(DateTime.Now.Year.ToString() + string.Format( "00",DateTime.Now.Month)))
						{
		...
		private void frmStorageInventory_Load(System.Object sender, System.EventArgs e)
		{
