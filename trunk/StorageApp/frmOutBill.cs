...
{
	public class frmOutBill : System.Windows.Forms.Form
	{
		
		public string CurrentOutBillTypeCode;
		...
		internal System.Windows.Forms.ComboBox cbDepartment;
		...
		[System.Diagnostics.DebuggerStepThrough()]private void InitializeComponent()
		{
			...
			base.Closed += new System.EventHandler(frmOutBill_Closed);
			System.Resources.ResourceManager resources = new System.Resources.ResourceManager(typeof(frmOutBill));
			...
			this.cbDepartment.Location = new System.Drawing.Point(344, 72);
			this.cbDepartment.Name = "cbDepartment";
			this.cbDepartment.Size = new System.Drawing.Size(120, 20);
			...
			this.cbVendor.Size = new System.Drawing.Size(128, 20);
		...
		//初始化入库窗口
		public void init(string outbilltypcode)
		{
			//返回允许访问的仓库部门
			...
			DataTable deptlstlst;
			deptlstlst = rms_var.LoadDept();
			for (int i = 0; i <= deptlstlst.Rows.Count - 1; i++)
			{
				cbDepartment.Items.Add(deptlstlst.Rows[i]["DeptName"]);
			}
			...
			//获取当月相应类型单据的单据记录
			LoadCurrentMonthBill(outbilltypcode);
		...
		//返回当月出库单记录
		private void LoadCurrentMonthBill(string outtypecode)
		{
			...
			try
			{
				...
				selectCMD.CommandText = "SELECT * FROM outStorageBill where storagebilltypecode='" + outtypecode + "'";
		...
		private DataTable LoadCurrentBillMaterials(string billno)
		{
			...
			try
			{
				SqlCommand selectCMD = new SqlCommand();
				selectCMD.Connection = conn;
				selectCMD.CommandText = "SELECT MaterialCode,MaterialName,Unit,Quantity,Price,quantity*price as totalprice FROM view_OutStorageMaterials where billno='" + billno + "'";
				selectCMD.CommandTimeout = 30;
				SqlDataAdapter dbDA = new SqlDataAdapter();
				dbDA.SelectCommand = selectCMD;
				conn.Open();
		...
		//保存 采购入库 单据
		private bool SaveBill(string billno, string StorageBillTypeCode, string Outdate, string vendorcode, string DepCode, string OutDepCode, string Operator, bool Audited, string note)
			{
			System.Data.SqlClient.SqlConnection conn = new System.Data.SqlClient.SqlConnection();
			conn.ConnectionString = rms_var.ConnStr;
			try
			{
				...
				else
				{
					selectCMD.Connection = conn;
					selectCMD.CommandTimeout = 30;
					selectCMD.CommandText = "insert into outstoragebill " + "(billno,StorageBillTypeCode,outdate,vendorcode,DepCode,OutDepCode,Operator,note,Audited) " + "values (" + "'" + billno + "'" + "," + "'" + StorageBillTypeCode + "'" + "," + "'" + Outdate + "'" + "," + "'" + vendorcode + "'" + "," + "'" + DepCode + "'" + "," + "'" + OutDepCode + "'" + "," + "'" + Operator + "'" + "," + "'" + note + "'" + "," + "'" + Math.Abs(System.Convert.ToInt32(Audited)).ToString() + "'" + ")";
		...
		private bool SaveBillsMaterials(string billno)
		{
			...
			try
			{
				...
				{
                    selectCMD.CommandText = "insert into outstoragematerials (billno,MaterialCode,MaterialTypeCode,Quantity,Price,TotalPrice) " + "values (" + "'" + billno + "'" + "," + "'" + dgMaterials[i, 0] + "'" + "," + "'" + rms_var.GetTheMaterialTypeCode(dgMaterials[i, 0].ToString()) + "'" + "," + dgMaterials[i, 3].ToString() + "," + dgMaterials[i, 4].ToString() + "," + dgMaterials[i, 5].ToString() + ")";
				...
				if (cbVendor.Text != "")
				{
					switch (rms_var.GetStorageBillTypeCode(TextBox3.Text))
					{
						case "D": //直拨单
							...
							for (int i = 0; i <= dgMaterials.VisibleRowCount - 1; i++)
							{
								totalcost_1 = totalcost_1 + System.Convert.ToDouble(dgMaterials[i, 5]);
							}
							selectCMD.CommandText = "update vendor set stock_paying=stock_paying+" + totalcost_1.ToString() + " where vendorcode='" + rms_var.GetVendorCode(cbVendor.Text) + "'";
		...
		private bool DeleteBill(string billno)
		{
			...
			try
			{
				...
				selectCMD.CommandTimeout = 30;
				conn.Open();
				selectCMD.CommandText = "delete from outstoragebill where billno='" + billno + "'";
				selectCMD.ExecuteNonQuery();
				selectCMD.CommandText = "delete from outstoragematerials where billno='" + billno + "'";
				selectCMD.ExecuteNonQuery();
				return true;
		...
		private bool DeleteBillsMaterial(string billno, string materialcode)
		{
			...
			try
			{
				...
				selectCMD.CommandTimeout = 30;
				conn.Open();
				selectCMD.CommandText = "delete from outstoragematerials where billno='" + billno + "' and materialcode='" + materialcode + "'";
				selectCMD.ExecuteNonQuery();
				return true;
			}
			catch (Exception ex)
		...
		public void NewOutBill()
		{
			...
			{
				TextBox1.Text = rms_var.GenNewStorageBillNo(1, CurrentOutBillTypeCode); //0=入库单据 1=出库单据
			}
			else
			{
				TextBox1.Text = "";
			}
			TextBox3.Text = rms_var.GetStorageBillTypeName(CurrentOutBillTypeCode);
			switch (CurrentOutBillTypeCode)
			{
				case "A": //领料单 部门退仓库 不需要输入供应商,必须输入部门
				case "H":
					cbVendor.Enabled = false;
					cbDepartment.Enabled = true;
					break;
				case "D": //直拨单 部门退供应商 需要输入供应商,必须输入部门
				case "F":
		...
		private void frmOutBill_Closed(object sender, System.EventArgs e)
		{
			rms_var.frmTheOutBill = null;
		}
		...
		private void ToolBar1_ButtonClick(System.Object sender, System.Windows.Forms.ToolBarButtonClickEventArgs e)
		{
			switch (ToolBar1.Buttons.IndexOf(e.Button))
			{
				...
				case 1: //添加物品
					...
					if (cbVendor.Enabled && cbVendor.Text == "")
					{
					...
					if (cbInStorageDept.Enabled && cbInStorageDept.Text == "")
					{
						MessageBox.Show("仓库不能为空！");
						cbInStorageDept.Focus();
						cbInStorageDept.SelectAll();
						return;
					}
					if (cbDepartment.Enabled && cbDepartment.Text == "")
					{
					...
					switch (CurrentOutBillTypeCode)
					{
						...
						case "B":
						case "G":
							//向 领料单 ，直拨单，部门退仓库，部门退供应商，仓库退供应商  添加物品
							if (! CheckBox1.Checked)
							{
								...
								frmselmtrl.BillTypeCode = CurrentOutBillTypeCode;
								...
								if (frmselmtrl.DialogResult == DialogResult.OK)
								{
									if (frmselmtrl.dgSelMaterials.VisibleRowCount > 0)
									{
										...
										for (int i = 0; i <= frmselmtrl.SelectedMaterials.Rows.Count - 1; i++)
										{
											...
											rowVals[3] = Convert.ToDecimal(frmselmtrl.SelectedMaterials.Rows[i][3]);
                                            rowVals[4] = frmselmtrl.SelectedMaterials.Rows[i][4] == "" ? 0 : Convert.ToDecimal(frmselmtrl.SelectedMaterials.Rows[i][4]);
											rowVals[5] = Convert.ToDecimal(frmselmtrl.SelectedMaterials.Rows[i][5]);
				...
				case 2: //保存当前单据
					...
					if (dgMaterials.VisibleRowCount > 0)
					{
						if (! CheckBox1.Checked)
						{
							if (SaveBill(TextBox1.Text, CurrentOutBillTypeCode, DateTimePicker1.Value.ToString(), "", rms_var.GetDeptCode(cbDepartment.Text), rms_var.GetDeptCode(cbInStorageDept.Text), Label9.Text, false, TextBox2.Text))
								{
								//添加单据物品列表
								if (SaveBillsMaterials(TextBox1.Text))
								{
									LoadCurrentMonthBill(CurrentOutBillTypeCode);
									CurrentBillRecNo = BillCount;
				...
				case 4: //删单据
					if (TextBox1.Text != "")
					{
						if (! CheckBox1.Checked)
						{
							if (MessageBox.Show("确定要删除当前单据吗？" +"\r\n"+ "注意：删除单据将会影响部门成本的核算！", "提示", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) == DialogResult.Yes)
							{
				...
				case 7: //审核
					if (CurrentBillRecNo != -1 && CurrentBillRecNo != BillCount)
					{
						if (dgMaterials.VisibleRowCount > 0)
						{
				...
				case 9: //前一单据
					...
					if (CurrentBillRecNo != -1)
					{
						...
						Label9.Text = BillList.Rows[CurrentBillRecNo][8].ToString();
						TextBox2.Text = BillList.Rows[CurrentBillRecNo][9].ToString();
						cbDepartment.Text = rms_var.GetDeptName(BillList.Rows[CurrentBillRecNo][3].ToString());
						CheckBox1.Checked = BillList.Rows[CurrentBillRecNo][11]=="1"?true:false;
				...
				case 12:
					if (dgMaterials.VisibleRowCount > 0)
					{
						...
						{
							if (! PrintReport("SELECT MaterialCode,MaterialName,Unit,Quantity,Price,name,quantity*price as totalprice FROM view_OutStorageMaterials where billno='" + TextBox1.Text + "'"))
							{
		...
		{
			if (report_var.outbill_cr == null)
			{
				report_var.outbill_cr = new ReportDocument();
			}
			else
			{
				report_var.outbill_cr.Close();
				report_var.outbill_cr = null;
				report_var.outbill_cr = new ReportDocument();
			}
			//检测报表文件是否存在
			if (File.Exists(report_var.GetReportPath() + report_var.outbill_rpt_filename))
			{
				report_var.outbill_cr = report_var.LoadReport(report_var.GetReportPath() + report_var.outbill_rpt_filename, sql, "view_OutStorageMaterials");
				
				TableLogOnInfo logOnInfo = new TableLogOnInfo();
				int i;
				
				// 对报表中的每个表依次循环。
				for (i = 0; i <= report_var.outbill_cr.Database.Tables.Count - 1; i++)
				{
					// 设置当前表的连接信息。
					...
					report_var.outbill_cr.Database.Tables[i].ApplyLogOnInfo(logOnInfo);
				}
				...
				crParameterFieldDefinitions = report_var.outbill_cr.DataDefinition.ParameterFields;
				
				crParameterFieldDefinition = crParameterFieldDefinitions["title"];
				crParameterValues = crParameterFieldDefinition.CurrentValues;
				crParameterDiscreteValue = new ParameterDiscreteValue();
				crParameterDiscreteValue.Value = "出库单";
				...
				crParameterFieldDefinition = crParameterFieldDefinitions["outbilltype"];
				crParameterValues = crParameterFieldDefinition.CurrentValues;
				crParameterDiscreteValue = new ParameterDiscreteValue();
				...
				crParameterFieldDefinition = crParameterFieldDefinitions["outbilldate"];
				crParameterValues = crParameterFieldDefinition.CurrentValues;
				crParameterDiscreteValue = new ParameterDiscreteValue();
				...
				crParameterFieldDefinition = crParameterFieldDefinitions["outstoragedept"];
				crParameterValues = crParameterFieldDefinition.CurrentValues;
				crParameterDiscreteValue = new ParameterDiscreteValue();
				...
				crParameterValues = crParameterFieldDefinition.CurrentValues;
				crParameterDiscreteValue = new ParameterDiscreteValue();
				crParameterDiscreteValue.Value = cbDepartment.Text;
				crParameterValues.Add(crParameterDiscreteValue);
				crParameterFieldDefinition.ApplyCurrentValues(crParameterValues);
				...
				report_var.outbill_cr.PrintToPrinter(1, true, 1, 1);
