...
namespace TouchScreen
{
	namespace MySpace
	{
		public class FoodTastesButtonList : System.Collections.CollectionBase
		{
			...
			public void AddNewButton(string taste)
			{
			...
			public FoodTastesButtonList(System.Windows.Forms.Panel host, int btnWidth, int btnHeight, Color btnOrignalBackColor, Color btnSelectedBackColor, int btnCountPerLine)
				{
			...
			public void ClickHandler(object sender, System.EventArgs e)
			{
				...
				for (int i = 0; i <= this.Count - 1; i++)
				{
					if (((System.Windows.Forms.Button) (this.List[i])).Tag.ToString() == "1")
					{
						if (foodtastes.Trim() == "")
						{
							foodtastes = ((System.Windows.Forms.Button) (this.List[i])).Text;
						}
						else
						{
							foodtastes = foodtastes + "," + ((System.Windows.Forms.Button) (this.List[i])).Text;
						}
					}
				}
				rms_var.frmAddFood.FoodTaste = foodtastes;
				rms_var.frmAddFood.txtTaste.Text = foodtastes;
