	...
	public class frmTableType : System.Windows.Forms.Form
	{
		...
		private void ExeDbProc(string begintime, string endtime)
		{
			...
			try
			{
				...
				MysqlProc.CommandText = "proc_TableType_sum";
		...
		private void Button1_Click(System.Object sender, System.EventArgs e)
		{
			...
			string sqlstr;
			sqlstr = "select * from tabletype_sum tabletypename";
			
			ParameterFieldDefinitions crParameterFieldDefinitions;
			ParameterFieldDefinition crParameterFieldDefinition;
			ParameterValues crParameterValues;
			ParameterDiscreteValue crParameterDiscreteValue;
			ReportDocument cr_tabletype_tmp = new ReportDocument();
			
			//检测报表文件是否存在
			if (File.Exists(rms_var.GetReportPath() + report_var.tabletype_rpt_filename))
			{
				cr_tabletype_tmp = report_var.LoadReport(rms_var.GetReportPath() + report_var.tabletype_rpt_filename, sqlstr, "TableType_sum");
			}
			else
			{
			...
			crParameterFieldDefinitions = cr_tabletype_tmp.DataDefinition.ParameterFields;
			...
			CrystalReportViewer1.ReportSource = cr_tabletype_tmp;
